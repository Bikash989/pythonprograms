...
abc = [a, b, c] # list of lists

 # find min from abc using list comprehension
min_abc = min([y for val in abc for y in val])

upperlimit = 20
max_abc = max([y for val in abc for y in val if y < upperlimit])

#dynamic limits
axis.set_ylim(min_abc, max_abc)
